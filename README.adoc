...
Details about each of these tools is provided in their respective READMEs (click the above links). The READMEs describe how each of these tools can be configured. Example configuration files are available link:{repo-home}/example_configs/[here].
...
- Configure the labManager admin-server to point it to the Theopenem instance and to where the institute's LDAP infrastructure can be found, see link:{repo-home}/example_configs/admin_server.yaml[the example configuration file].
